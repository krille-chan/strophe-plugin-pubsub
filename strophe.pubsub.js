...
(function () {
	...
	OnMucInvitation: null,
			...
			HandlePubsubEvent: function(iq){
				if(this.OnPubsubEvent == null) return;
				if(iq.querySelector("items") != null){
					var ansList = [];
					var ansNodes = iq.querySelectorAll("item");
					for(i=0; i<ansNodes.length;i++)
						if(iq.getAttribute("id") == "items1")
							ansList.push({
								"id" : ansNodes[i].getAttribute("id"),
								"title" : ansNodes[i].querySelector("title").innerHTML,
								"summary" : ansNodes[i].querySelector("summary").innerHTML,
								"link" : ansNodes[i].querySelector("link").innerHTML,
								"itemid" : ansNodes[i].querySelector("itemid").innerHTML,
								"published" : ansNodes[i].querySelector("published").innerHTML,
								"updated" : ansNodes[i].querySelector("updated").innerHTML,
								"content" : ansNodes[i].querySelector("content").innerHTML,
							});
						else
							ansList.push({
								"id" : ansNodes[i].getAttribute("id")
